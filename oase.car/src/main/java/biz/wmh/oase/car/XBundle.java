 ...
 * of this class represents the context bundle hosting the OSGi framework.
 * An instance of this class is valid only when the OSGi bundle is in a state
 * between STARTING and STOPPED.
 *
 * @version 2.0.0 14.01.2026 16:28:27
 */
public class XBundle {
	...
	/**
	 * Das Wiring der Bundle Packages wird durchgeführt.
	 */
	public void refresh() {
		// 1. Hol dir das System-Bundle (ID 0)
		BundleContext l_ctx = getBundleContext();
		Bundle l_osgi = l_ctx.getBundle(0);

		// 2. Erhalte das FrameworkWiring-Interface
		FrameworkWiring l_wire = l_osgi.adapt(FrameworkWiring.class);

		// 3. Führe den Refresh durch
		Bundle l_me = unwrapped();
		l_wire.refreshBundles(Collections.singleton(l_me));
	}
